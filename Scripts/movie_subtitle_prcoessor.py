...
if path.exists(diff_csv_file):
    df_freq = pd.read_csv(diff_csv_file)  
else:
    index = -1
    ...
    df_freq = pd.DataFrame.from_dict(data=new_movie_word_frequency_dict, orient='index', columns=['word', 'freq', 'difficulty'])
    df_freq = df_freq.sort_values(by=['difficulty'], ascending=False)
    df_freq.to_csv(diff_csv_file)
...
count = 0
difficulty_top_n = []
most_difficult_words = df_freq['word']
for index, word in most_difficult_words.iteritems():
    #meaning = dictionary.translate(word, 'fr')
    meaning = dictionary.meaning(word, disable_errors=True)
    if meaning != None:
        count += 1
        meaningful_words[word] = meaning
        difficulty_top_n.append(df_freq.loc[df_freq['word'] == word, 'difficulty'].iloc[0])
    if count == int(top_n):
        break
...
for key, value in meaningful_words.items():
    key_difficulty = df_freq.loc[df_freq['word'] == key, 'difficulty'].iloc[0]
